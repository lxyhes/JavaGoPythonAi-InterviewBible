  ...
  <transition name="celebration-pop">
    <aside v-if="celebration" class="celebration-shell" @click="dismiss">
      <div class="celebration-card" @click.stop>
        ...
        <div class="actions">
          <router-link class="primary-btn" to="/dashboard" @click="dismiss">View progress</router-link>
          <button type="button" class="ghost-btn" @click="dismiss">Continue</button>
        </div>
      </div>
    </aside>
  </transition>
...
const badgeLabel = computed(() => {
  ...
  if (celebration.value.type === 'goal') return 'Daily Win'
  if (celebration.value.type === 'level') return 'Level Up'
...
const dismiss = () => {
  if (!celebration.value) return
  learningStore.dismissCelebration(celebration.value.id)
}
